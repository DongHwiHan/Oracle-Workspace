...
SELECT
    ...
    ( SALARY + SALARY * NVL(BONUS, 0) ) * 12 - ( SALARY * 12 ) * 0.3 실수령액
FROM
    EMPLOYEE;
...
FROM
    EMPLOYEE
WHERE
    ( SALARY + SALARY * NVL(BONUS, 0) ) * 12 - ( SALARY * 12 ) * 0.3 >= 50000000;

-- 9. EMPLOYEE테이블에 월급이 4000000이상이고 JOB_CODE가 J2인 사원의 전체 내용 조회4
SELECT
    *
FROM
    EMPLOYEE
...
FROM
    EMPLOYEE
WHERE
    EMAIL LIKE '___^%' ESCAPE '^'
    AND DEPT_CODE = 'D9'
    OR DEPT_CODE = 'D6'
    AND HIRE_DATE BETWEEN '90/01/01' AND '03/01/01'
    AND SALARY >= 2700000;
...
SELECT
    EMP_NAME,
    EMP_NO,
    EXTRACT(YEAR FROM TO_DATE(SUBSTR(EMP_NO, 1, 2),
        'YY')) - 2000 생년,
    EXTRACT(MONTH FROM TO_DATE(SUBSTR(EMP_NO, 1, 4),
        'YYMM'))      생월,
...
SELECT
    EMP_NAME                        사원명,
    ROUND(ABS(HIRE_DATE - SYSDATE)) 근무일수1,
    ROUND(SYSDATE - HIRE_DATE)      근무일수2
FROM
    EMPLOYEE
ORDER BY
    근무일수1 DESC;
...
FROM
    EMPLOYEE
WHERE
    TO_CHAR(SYSDATE, 'RRRR') - TO_CHAR(HIRE_DATE, 'RRRR') >= 20;

-- 20. EMPLOYEE 테이블에서 사원명, 급여 조회 (단, 급여는 '\9,000,000' 형식으로 표시)
SELECT
    EMP_NAME                      사원명,
    TO_CHAR(SALARY, 'L9,999,999') 급여
FROM
    EMPLOYEE;
...
SELECT
    ...
    TO_CHAR(TO_DATE(SUBSTR(EMP_NO, 1, 6),
        'RRMMDD'),
            'RR"년 "MM"월 "DD"일"') 생년월일,
    TO_CHAR(SYSDATE, 'RRRR') - TO_CHAR(TO_DATE(SUBSTR(EMP_NO, 1, 2),
        'RRRR'),
                                       'RRRR')                  "나이(만)"
FROM
    EMPLOYEE;
...
-- 주민번호 앞자리와 뒷자리의 합 조회
 ...
 SUBSTR(EMP_NO,8,14)앞자리, SUBSTR(EMP_NO,1,6) 뒷자리 + SUBSTR(EMP_NO,8,14) "앞자리와 뒷자리의 합"
 FROM EMPLOYEE
 WHERE EMP_ID = '201';
...
SELECT
    EMP_DEPT                           부서코드,
    SUM((SALARY * NVL(BONUS, 0)) * 12) "보너스 포함 연봉"
FROM
    EMPLOYEE
WHERE
    DEPT_CODE = 'D5';
